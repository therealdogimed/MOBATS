    ...
    for (const symbol of watchlist) {
      try {
        ...
        const decision = await aiAgent.analyzeAndDecide({
          ...
          symbol,
          currentPrice, // Real market price
        ...
        if (decision.action === "buy" && decision.quantity > 0) {
          positionTracker.recordPosition({
            ...
            entryPrice: currentPrice, // Real price
    ...
    for (const position of positions) {
      try {
        ...
        if (decision.action === "sell") {
          positionTracker.closePosition(position.positionId, decision.reasoning)
      ...
      try {
        ...
        if (decision.action === "buy" && decision.quantity > 0 && decision.confidence > 70) {
          positionTracker.recordPosition({
            ...
            symbol,
            qty: decision.quantity,
            entryPrice: currentPrice, // Live price
            currentPrice, // Live price
            strategyId: strategy.id,
            strategyName: strategy.name,
