...
export async function syncAlpacaData() {
  try {
    ...
    for (const alpacaBroker of alpacaBrokers) {
      // CRITICAL FIX: Skip paper accounts when verifying for live trading
      if (alpacaBroker.mode === "paper" && process.env.ALPACA_MODE === "live") {
        console.log("[v0] bot-state: Skipping paper account in live trading mode", {
      ...
      if (!verification.success) {
        ...
        // CRITICAL FIX: Save credentials even if verification fails
        if (typeof window !== "undefined") {
